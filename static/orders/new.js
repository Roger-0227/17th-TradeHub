  ...
  formsetItems.addEventListener("input", (event) => {
    if (
      event.target.matches(
        'input[name$="-ordered_quantity"], input[name$="-sale_price"]'
      )
    ) {
      const row = event.target.closest("fieldset");
      const orderedQuantityInput = row.querySelector(
        'input[name$="-ordered_quantity"]'
      );
      ...
      if (orderedQuantityInput && salePriceInput && subtotalInput) {
        subtotalInput.value = Math.round(
          orderedQuantityInput.value * salePriceInput.value
        );
        updateTotalAmount();
      }
    }
    ...
    document
      ...
      .forEach((productSelect) => {
        productSelect.addEventListener("change", function () {
          ...
          const orderdQuantityInput = fieldset.querySelector(
            'input[name$="-ordered_quantity"]'
          );
          ...
          fetch(`/orders/load_product_info/?id=${productId}`)
            ...
            .then((data) => {
              ...
              subtotalInput.value =
                orderdQuantityInput.value * salePriceInput.value;
              updateTotalAmount();
            });
        });
      });
