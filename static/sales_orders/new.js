document.addEventListener("DOMContentLoaded", () => {
  ...
  const formSubmit = document.querySelector("[name='submit']");
  const addItemButton = document.getElementById("add-item");
  const totalForms = document.getElementById("id_items-TOTAL_FORMS");
  const salePriceInput = document.querySelector('input[name$="-sale_price"]');
  const stockQuantitySelects = document.querySelectorAll(
    'select[name$="-stock_quantity"]'
  );
  const orderedQuantityInput = document.querySelector(
    'input[name$="-ordered_quantity"]'
  );
  const shippedQuantityInput = document.querySelector(
    'input[name$="-shipped_quantity"]'
  );
  const orderedQuantityInputs = document.querySelectorAll(
    'input[name$="-ordered_quantity"]'
  );
  const shippedQuantityInputs = document.querySelectorAll(
    'input[name$="-shipped_quantity"]'
  );
  ...
  stockQuantitySelects.forEach((stockQuantitySelect) => {
    stockQuantitySelect.disabled = true;
  });

  formSubmit.addEventListener("click", () => {
    stockQuantitySelects.forEach((stockQuantitySelect) => {
      stockQuantitySelect.disabled = false;
    });
  });
  ...
  addItemButton.addEventListener("click", () => {
    const newItem = document
      .querySelector("#formset-items fieldset")
      .cloneNode(true);
  ...
  formsetItems.addEventListener("input", (event) => {
    if (
      event.target.matches(
        'input[name$="-shipped_quantity"], input[name$="-sale_price"]'
      )
    ) {
      const row = event.target.closest("fieldset");
      const shippedQuantityInput = row.querySelector(
        'input[name$="-shipped_quantity"]'
      );
      ...
      if (shippedQuantityInput && salePriceInput && subtotalInput) {
        subtotalInput.value = Math.round(
          shippedQuantityInput.value * salePriceInput.value
        );
        updateTotalAmount();
      }
    }
  ...
  function toggleFormItems(disabled) {
    document
      .querySelectorAll('[id^="id_items-"][id$="-product"]')
  ...
  function fetchClientInfo(clientId) {
    fetch(`/orders/load_client_info/?client_id=${clientId}`)
      ...
      .then((data) => {
        ...
        document.getElementById("id_client_address").value =
          data.client_address;
    ...
    formsetItems.querySelectorAll("fieldset").forEach((fieldset, index) => {
      fieldset.querySelectorAll("input, select").forEach((element) => {
        ...
        if (id)
          element.setAttribute(
            "id",
            id.replace(/id_items-\d+-/, `id_items-${index}-`)
          );
      });
    });
    ...
    document
      ...
      .forEach((productSelect) => {
        productSelect.addEventListener("change", function () {
          ...
          const salePriceInput = fieldset.querySelector(
            'input[name$="-sale_price"]'
          );
          const stockQuantitySelect = fieldset.querySelector(
            'select[name$="-stock_quantity"]'
          );
          const shippedQuantityInput = fieldset.querySelector(
            'input[name$="-shipped_quantity"]'
          );
          const orderedQuantityInput = fieldset.querySelector(
            'input[name$="-ordered_quantity"]'
          );
          const subtotalInput = fieldset.querySelector(
            'input[name$="-subtotal"]'
          );
